...
struct GDExtension {
            ...
            for platform in platforms {
                if platform.separateArchs {
                    for arch in archs {
                        ...
                        if let runtime = platformDependencies["\(platform.directory(for: arch))"] {
                            for libName in runtime {
                                let library = "\(binLocation)/\(platform.directory(for: arch)).\(mode.rawValue).\(libName)"
                    ...
                    if let runtime = platformDependencies[platform.directory(for: nil)] {
                        for libName in runtime {
                            let library = "\(binLocation)/\(platform.directory(for: nil)).\(mode.rawValue).\(libName)"
                            targetDependencies[library] = ""
                        }
                    }
                    dependencies[target] = targetDependencies
    ...
    func tscnEntry(
        ...
        let baseLocation = "\(binLocation)/\(platform.directory(for: arch)).\(mode.rawValue)"
